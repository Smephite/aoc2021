...
class Node:
    ...
    def __repr__(self) -> str:
        return f"Node[{self.position}, {self.risk_level}, {self.distance}, {self.previous.position if self.previous is not None else None}, {self.visited}]"
...
for y in range(len(input)):
    for x in range(len(input[0])):
        pos = (x, y)
        node = Node(pos, int(input[y][x]))
        nodes[pos] = node

nodes[(0, 0)].distance = 0

pos_output = (len(input)-1, len(input[0])-1)
